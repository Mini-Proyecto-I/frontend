...
export default function ProgressPage() {
  ...
  const stats = useMemo(() => {
    // Calculamos estadísticas basadas en filteredActivities para mayor consistencia
    let done = 0;
    let total = 0;
    let hoursDone = 0;

    filteredActivities.forEach((activity) => {
      activity.matchingSubtasks?.forEach((st: Subtask) => {
        total++;
  ...
  const handleCompleteTask = async (
    ...
    try {
      ...
      setBackendProgress((prev: any) => {
        ...
        const newDone = Math.max(0, (prev.total_subtasks_done || 0) + delta);
        const total = prev.total_subtasks || 0;
        return {
          ...prev,
          total_subtasks_done: newDone,
          completion_percent: total > 0 ? (newDone / total) * 100 : 0
        };
      });
  ...
  return (
    <div className="flex flex-col gap-8 max-w-[1400px] w-full mx-auto px-4 sm:px-6 lg:px-10 pb-10 mt-6 lg:mt-10">
          ...
          <div className="bg-[#111827] border border-slate-800/60 rounded-3xl p-6 shadow-xl shadow-black/20">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-white">{stats.done}/{stats.total}</span>
              </div>
              <div className="flex justify-between text-sm">
      ...
      <button
        ...
        className="fixed bottom-6 left-6 z-40 w-12 h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg hover:shadow-xl transition-all hover:scale-110 flex items-center justify-center p-0"
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111827] border border-slate-800 rounded-3xl p-8 flex flex-col items-center text-center shadow-2xl max-w-[520px] w-full relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
            ...
            <h3 className="text-2xl font-extrabold text-white mb-4 tracking-tight">
              ¿Cómo funciona la vista de <span className="text-blue-400">Progreso</span>?
            </h3>
            ...
            <div className="flex flex-col gap-5 text-left w-full mb-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-emerald-400 text-[15px] font-bold leading-snug mb-1">Actividades y subtareas</p>
                  <p className="text-slate-400 text-[13px] leading-relaxed">
                    Se muestran las actividades con subtareas existentes en el periodo de tiempo seleccionado.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="text-blue-400 text-[15px] font-bold leading-snug mb-1">Progreso de actividad</p>
                  <p className="text-slate-400 text-[13px] leading-relaxed">
                    El progreso de cada actividad se calcula con el total de sus tareas originales.
                  </p>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="bg-purple-500/10 p-2.5 rounded-xl border border-purple-500/20 mt-0.5">
                  <Zap className="w-5 h-5 text-purple-400 shrink-0" />
                </div>
                <div>
                  <p className="text-purple-400 text-[15px] font-bold leading-snug mb-1">Progreso del periodo</p>
                  <p className="text-slate-400 text-[13px] leading-relaxed">
                    El progreso circular de arriba se calcula solo con las tareas de todas las actividades que se encuentren dentro del periodo (hoy, semana, mes).
                  </p>
                </div>
              </div>

              <div className="flex items-start gap-4">
                <div className="bg-slate-500/10 p-2.5 rounded-xl border border-slate-500/20 mt-0.5">
                  <Search className="w-5 h-5 text-slate-400 shrink-0" />
                </div>
                <div>
                  <p className="text-slate-400 text-[15px] font-bold leading-snug mb-1">Filtros y extras</p>
                  <p className="text-slate-400 text-[13px] leading-relaxed">
                    Puedes aplicar filtros, revisar futuras entregas y consultar el historial de cada tarea.
                  </p>
                </div>
              </div>
            </div>
      ...
      {showCircleHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111827] border border-slate-800 rounded-3xl p-6 flex flex-col items-center text-center shadow-2xl max-w-[400px] w-full relative animate-in fade-in zoom-in-95 duration-200">
            <div className="w-12 h-12 bg-blue-500/10 border border-blue-500/30 rounded-full flex items-center justify-center mb-4">
              <BarChart3 className="w-6 h-6 text-blue-400" />
            </div>
            <h3 className="text-xl font-bold text-white mb-2">Cálculo de progreso</h3>
            <p className="text-slate-400 text-sm leading-relaxed mb-6">
              Este progreso representa la relación entre las tareas <span className="text-emerald-400 font-semibold">completadas</span> y el <span className="text-white font-semibold">total de tareas</span> planificadas para el periodo seleccionado.
            </p>
            <Button
              onClick={() => setShowCircleHelp(false)}
              className="bg-slate-800 hover:bg-slate-700 text-white h-10 rounded-xl px-6"
            >
              Cerrar
            </Button>
          </div>
